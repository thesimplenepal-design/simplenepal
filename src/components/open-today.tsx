...
const TONE: Record<OpenState['status'], string> = {
  ...
  closed: 'border-l-[--color-crimson] bg-[--color-crimson-soft]',
  maybe: 'border-l-amber-500 bg-amber-50/70 dark:bg-amber-950/20',
}

const DOT: Record<OpenState['status'], string> = {
  open: 'bg-emerald-600', closed: 'bg-[--color-crimson]', maybe: 'bg-amber-500',
}
...
export function OpenToday({ state, schedule }: { state: OpenState; schedule: Schedule }) {
  return (
    <div className={`mt-5 rounded-xl border border-[--color-line] border-l-[3px] px-4 py-3.5 ${TONE[state.status]}`}>
      <div className="flex items-center gap-2.5 flex-wrap">
        ...
        {state.hours && (
          <span className="text-[13.5px] text-[--color-ink-2] ml-auto tabular-nums">{state.hours}</span>
        )}
      </div>

      {state.detail && (
        <p className="text-[13.5px] text-[--color-ink-2] mt-1.5 mb-0">{state.detail}</p>
      )}

      <p className="text-[12px] text-[--color-ink-3] mt-2 mb-0">
