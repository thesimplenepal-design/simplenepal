...
export default async function GovIndex() {
  ...
  return (
    <Container className="py-10">
      ...
      <p className="text-[13.5px] text-[var(--color-ink-3)] mb-5 max-w-2xl">
        ...
        <strong className="text-[var(--color-ink-2)]"> not yet confirmed against the Nepal Gazette</strong> —
        ...
        {ministries.map((m) => (
          <li key={m.id}>
            <Link href={`/gov/${m.slug}`}
              className="flex items-start gap-3 no-underline rounded-xl border border-[var(--color-line)]
                         bg-[var(--color-surface)] px-4 py-3 hover:border-[var(--color-crimson)] transition-colors h-full">
              <div className="min-w-0">
                <div className="font-medium text-[14.5px] leading-snug">{m.nameEn}</div>
                {m.nameNe && <div className="ne text-[13px] text-[var(--color-ink-3)] mt-0.5">{m.nameNe}</div>}
                {m.depts > 0 && (
      ...
      {superseded.length > 0 && (
        <>
          ...
          <ul className="space-y-2">
            {superseded.map((s) => (
              <li key={s.slug}
                  className="rounded-xl border border-[var(--color-line)] bg-[var(--color-surface-2)] px-4 py-3">
                <div className="text-[14.5px] font-medium text-[var(--color-ink-2)] line-through decoration-1">
                  {s.nameEn}
                </div>
                {s.nameNe && <div className="ne text-[12.5px] text-[var(--color-ink-3)]">{s.nameNe}</div>}
                <div className="text-[13px] text-[var(--color-ink-2)] mt-1.5">
                  {s.note}{' '}
                  {s.successorSlug && (
